    ...
    if os.path.isdir(folder_path):
        ...
        with open(accumulate_csv_file_path, mode='w', newline='') as csvfile:
            ...
            for csv_file in csv_files:
                ...
                for _, row in df.iterrows():
                    array_representation = np.array(row)[1:] 
                    utility, friendship_utility = calculate_utility(students, row['student'], array_representation, allocation, friendship_weight=1)

                    students[row['student']].set_course_utility(utility)
                    students[row['student']].set_friends_utility(friendship_utility)
                    students[row['student']].set_total_utility(utility+friendship_utility)
